...
# ============================================================
# BARRERA LIVE DE ESTA FASE
# ============================================================
#
# El bot conserva TODO el flujo del bot PAPER:
# Gmail, señales, estados, PROCESSING, reconciliación,
# notificaciones, controles y reconexiones.
#
# El envío real sigue bloqueado dentro de LiveExecutor
...
def create_ib_executor():
    ...
    print(
        "CONECTANDO CON IB GATEWAY LIVE"
    )
    ...
    # Validación adicional del contrato LIVE antes de continuar.
    if hasattr(
        app,
        "validate_contract"
    ):

        contract = app.validate_contract()
        ...
        if not contract.get(
            ...
            log_error(
                "El contrato FDXM LIVE no supera la validación."
            )
...
def create_mail_connection():
    ...
    # Verificación explícita de que la sesión IMAP responde.
...
def process_control_messages(
    ...
    for email_uid in control_uids:
        ...
        # ----------------------------------------------------
        #
        # Se reconoce y enruta la orden manual igual que el
        # resto del sistema. En esta fase, LiveExecutor
        # devolverá BLOCKED antes de placeOrder() porque
        # VALIDATION_ONLY permanece activo.
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # El sistema de CONFIRMACIÓN por código fue retirado.
        # No se ejecuta ninguna segunda fase de confirmación.
        # ----------------------------------------------------
        ...
        if command_type == "CONFIRM":
            ...
            result = {
                ...
                "command": "CONFIRM",
                "subject":
                    "DAX BOT - Confirmación no necesaria",
                "body": (
                    "El sistema de confirmación por código "
                    "no está activo.\n\n"
                    "No se ha realizado ninguna operación."
                )
            }
            ...
            log_warning(
                f"Confirmación recibida pero no necesaria | "
...
def run():
    ...
    log_info(
        "DAX BOT iniciado en modo LIVE | "
        "VALIDATION_ONLY activo."
    )
